...
import { generateYearlyPulseSVG, type SvgTheme } from "@/lib/github-stats-svg";
...
export async function GET(
  ...
  const cacheKey = `pulse-yearly:${username.toLowerCase()}:${theme}`;

  try {
    const svg = await getCachedSVG(cacheKey, TTL.LONG, async () => {
      const stats = await fetchAdvancedGitHubStats(username);
      if (!stats) return null;
      return generateYearlyPulseSVG(stats.activityDays, theme);
    });
    ...
    console.error("Error generating yearly pulse:", error);
